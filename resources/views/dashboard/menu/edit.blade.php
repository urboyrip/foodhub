  ...
  <form method="post" action="/dashboard/menu/{{ $menu->id }}" enctype="multipart/form-data">
    ...
    @method('put')
      ...
      <div class="mb-3">
        ...
        <input value="{{ old('name',$menu->name) }}" name="name" type="text" class="form-control"  >
      </div>

      <div class="mb-3">
        <label for="description" class="form-label ">Description</label>
        <input type="hidden" name="description" id="description" class="@error('description') is-invalid @enderror" required value="{{ old('description',$menu->description) }}" >
        <trix-editor input="description"></trix-editor>
      </div>

      <div class="mb-3">
          <label class="form-label">price</label>
          <input value="{{ old('price',$menu->price) }}" name="price" type="number" class="form-control"  >
        </div>

        <div class="mb-3">
          <label for="formFile" class="form-label">Foto Makanan</label>
          @if ($menu->picture)
          <img src="{{ asset('storage/'.$menu->picture) }}" class="img-preview img-fluid" src="" alt="">
          @else
